...
class FaissService:
    ...
    def update_index(self, index: int, vector: np.ndarray):
        if 0 <= index < self.vectors.shape[0]:
            self.vectors[index] = vector
            self.index = faiss.IndexFlatL2(self.vectors.shape[1])
            self.index.add(self.vectors)
            self.save_index()
        else:
            print(f"Index {index} out of bounds. Cannot update vector.")
    ...
    def remove_from_index(self, index: int):
        if 0 <= index < self.vectors.shape[0]:
            ...
            print(f"Index {index} out of bounds. Cannot remove vector.")
